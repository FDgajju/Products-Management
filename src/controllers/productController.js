...
let releaseProduct = async function (req, res) {
  try {
    ...
    if (!validate.isValidRequestBody(reqBody)) {
      res
        .status(400)
        .send({ status: false, message: 'request body is required' });
      return;
    }
    ...
    for (let i = 0; i < availableSizes.length; i++) {
      if (!validate.isValid(availableSizes[i])) {
        res.status(400).send({
          status: false,
          message: `${availableSizes[i]} is not valid size`,
        });
        return;
      }
    }
    ...
    for (let i = 0; i < availableSizes.length; i++) {
      if (!validate.isValidSize(availableSizes[i])) {
        res.status(400).send({
          status: false,
          message: `${availableSizes[i]} not a valid size please enter size Btween 'S', 'XS', 'M', 'X', 'L', 'XXL', 'XL'`,
        });
        return;
      }
    }
    ...
    if (findTitle) {
      res.status(403).send({
        status: false,
        message: 'product with this title already exist it must be unique',
      });
      return;
    }
...
const getProduct = async function (req, res) {
  try {
    if (
      req.query.size ||
      req.query.name ||
      req.query.priceGreaterThan ||
      req.query.priceLessThan
    ) {
      ...
      if (!getProductsList || getProductsList.length == 0) {
        ...
        res
          .status(200)
          .send({ status: true, message: 'Success', data: getProductsList });
      }
    } else {
      const getListOfProducts = await productModel
        .find({ isDeleted: false, deletedAt: null })
        .sort({ price: 1 });
      res
        .status(200)
        .send({ status: true, message: 'Success', data: getListOfProducts });
    }
  } catch (err) {
...
const getProductById = async function (req, res) {
  try {
    ...
    if (!findPro) {
      res.status(404).send({
        status: false,
        message: `product is not available with this ${id} id`,
      });
      return;
    }
  ...
  try {
    ...
    if (!productDetails) {
      return res.status(404).send({
        status: false,
        message: `productDetails not found with given productID`,
      });
    }
    ...
    if (requestBody.hasOwnProperty('title')) {
      ...
      if (isTitleAlreadyUsed) {
        res.status(400).send({
          status: false,
          message: `${title} these title is already registered`,
        });
        return;
      }
    }
    ...
    if (requestBody.hasOwnProperty('isFreeShipping')) {
      if (!validate.isValid(isFreeShipping)) {
        res.status(400).send({
          status: false,
          message: 'please enter valid isFreeShipping',
        });
        return;
      }
    }
    ...
    if (requestBody.hasOwnProperty('currencyId')) {
      if (!validate.isValid(currencyId)) {
        res
          .status(400)
          .send({ status: false, message: 'please enter valid currencyId' });
        return;
      }
    } //currencyFormat
    ...
    if (requestBody.hasOwnProperty('availableSizes')) {
      for (let i = 0; i < availableSizes.length; i++) {
        if (!validate.isValid(availableSizes[i])) {
          res.status(400).send({
            status: false,
            message: 'please enter valid availableSizes',
          });
          return;
        }
        ...
        if (isTitleAlreadyUsed.availableSizes.indexOf(availableSizes) !== -1) {
          res
            .status(400)
            .send({ status: false, message: 'this size already avalible' });
          return;
        }
    ...
    if (!validate.isValidRequestBody(requestBody)) {
      return res.status(400).send({
        status: false,
        message: 'No parameters passed. product unmodified',
      });
    }
...
const deleteProductByID = async (req, res) => {
  try {
    ...
    if (!findProduct) {
      ...
      const deleteData = await productModel.findOneAndUpdate(
        { _id: { $in: findProduct } },
