  ...
  try {
    ...
    if (reqParams === userId) {
      if (!validate.isValid(userId)) {
        res
          .status(400)
          .send({ status: false, message: 'enter valide user id' });
        return;
      }
      ...
      if (!validate.isValid(productID)) {
        res
          .status(400)
          .send({ status: false, message: 'enter valide productId' });
        return;
      }
      ...
      if (!validate.isValid(proQuantity)) {
        res
          .status(400)
          .send({ status: false, message: 'enter valide quantity' });
        return;
      }
      ...
      if (!checkPrice) {
        res
          .status(404)
          .send({ status: false, message: `${productID} id dose not exist` });
        return;
      }
      ...
      if (ifCartAlreadyCreated) {
        ...
        for (let i = 0; i < ifCartAlreadyCreated.items.length; i++) {
          ...
          if (convertStr === productID) {
            ...
            const updateExistingProduct = await cartModel.findOneAndUpdate(
              { userId: userId, 'items.productId': productID },
              {
                'items.$.quantity': find.quantity + proQuantity,
                totalPrice: proP + ifCartAlreadyCreated.totalPrice,
              },
              { new: true }
            );
            res.status(200).send({
              status: true,
              message: 'thanks for purchesing product have a great day',
        ...
        console.log(proP);
        const updateItems = await cartModel.updateOne(
          { userId },
          { $push: { items: items } }
        );
        const updateTPriceItems = await cartModel.findOneAndUpdate(
          { userId },
          {
            totalPrice: proP + ifCartAlreadyCreated.totalPrice,
            totalItems: ifCartAlreadyCreated.totalItems + items.length,
          },
          { new: true }
        );
        res.status(200).send({
...
const updateCart = async function (req, res) {
  try {
    ...
    const findQuant = findUserCart.items.find(
      (c) => c['productId'] == productId
    );
    if (!findQuant) {
      res.status(400).send({
        status: false,
        message: 'product does not  exist in this cart',
      });
      return;
    }
    ...
    if (removeProduct === 1) {
      ...
      let dec = await cartModel.findOneAndUpdate(
        { _id: cartId },
        { totalPrice: totalP - quantProPrice },
        { new: true }
      );
...
const getCart = async function (req, res) {
  try {
    ...
    res
      .status(200)
      .send({ status: true, message: 'User cart details', data: getCart });
  } catch (err) {
...
const deleteCart = async function (req, res) {
  try {
    ...
    if (!validate.isValidObjectId(reqParams)) {
      return res
        .status(400)
        .send({ status: false, message: `${reqParams} is not a valid userId` });
    }
    ...
    if (!isUserExist) {
      return res
        .status(400)
        .send({ status: false, message: `${reqParams} user not available` });
    }
    ...
    if (!isCartExist) {
      return res
        .status(400)
        .send({ status: false, message: `user don't have cart` });
    }
    ...
    if (isCartExist.items.length === 0) {
      return res.status(400).send({
        status: false,
        message: `product not available in existing cart`,
      });
    }
