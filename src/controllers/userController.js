...
let registerUser = async function (req, res) {
  try {
    ...
    if (files && files.length > 0) {
      ...
      res
        .status(400)
        .send({ status: false, message: 'please select profile image' });
    }
  } catch (error) {
...
const updateUserDetails = async (req, res) => {
  try {
    ...
    if (!validate.isValidRequestBody(requestBody)) {
      res.status(400).send({
        status: false,
        message: 'which filed do you want to update ?',
      });
      return;
    }
    ...
    if (!UserFound) {
      return res
        .status(404)
        .send({ status: false, message: `User not found with given UserId` });
    }

    if (!TokenDetail === userId) {
      res.status(400).send({
        status: false,
        message: 'userId in url param and in token is not same',
      });
    }
    ...
    if (requestBody.hasOwnProperty('fname')) {
      const isfnameAlreadyUsed = await userModel.findOne({
        fname: fname,
      });
      ...
      if (isfnameAlreadyUsed) {
        res.status(400).send({
          status: false,
          message: `${fname} fname  is already exists`,
        });
        return;
      }
    ...
    if (requestBody.hasOwnProperty('lname')) {
      const islnameAlreadyUsed = await userModel.findOne({
    ...
    if (requestBody.hasOwnProperty('phone')) {
      ...
      if (isphoneAlreadyUsed) {
        res.status(400).send({
          status: false,
          message: `${requestBody.phone} phone number is already registered`,
        });
        return;
      }
      if (!validate.isValid(phone)) {
    ...
    if (requestBody.hasOwnProperty('email')) {
      if (
        !/^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/.test(requestBody.email)
      ) {
        res.status(400).send({
          status: false,
      ...
      if (isEmailAlreadyUsed) {
        res.status(400).send({
          status: false,
          message: `${requestBody.email} email address is already registered`,
        });
        return;
      }
    }
    ...
    if (password) {
      ...
      if (!(password.length >= 8 && password.length <= 15)) {
        res.status(400).send({
          status: false,
          message: 'password should  between 8 and 15 characters',
        });
        return;
      }
    }
    ...
    const upatedUser = await userModel.findOneAndUpdate(
      { _id: userId },
      UpdateData,
      { new: true }
    );
